...
namespace Core.Base
{
    public class InputStatusFrm:System.Windows.Forms.Form
    {
        ...
        // 保存候选框的上一次位置
        private static int savedLeft = 0;
        private static int savedTop = 0;
        private static bool positionSaved = false;

        // 鼠标拖动相关
        private bool isDragging = false;
        private Point dragStartPoint;
        private Point dragStartWindowPos;
        ...
        public static void SendText(string message, string mcode,bool keysend = false)
        {
            ...
            if (Input.IsChinese == 1 && LastLinkString.Length > 8) LastLinkString = LastLinkString.Substring(LastLinkString.Length-8);
            if (Input.IsChinese == 1 && LastLinkCodeString.Length > 8) LastLinkCodeString = LastLinkCodeString.Substring(LastLinkCodeString.Length - 8);
        ...
        private string ExtractWord(string text)
        {
            ...
            if (bracketIndex > 0)
            {
                return text.Substring(0, bracketIndex);
            }
            return text;
        }
            ...
            for (int i = 0; i < InputStatusFrm.cachearry.Length; i++)
            {
                if (pos == 0)
                {
                    if (string.IsNullOrEmpty(InputStatusFrm.cachearry[i]))
                    {
                        if (LSView)
                        {
                            string originalText = InputStatusFrm.cachearry[0].Split('|')[1];
                            string wordOnlyLS = ExtractWord(originalText);
                            if (!InputMode.imgsend)
                                for (int j = 0; j < StrLength(wordOnlyLS); j++)
                                    InputStatusFrm.SendText("{BACKSPACE}", "", true);
                            else
                            {
                                for (int j = 0; j < wordOnlyLS.Length; j++)
                                {
                        ...
                        string textToSend = InputStatusFrm.cachearry[i - 1].Split('|')[1];
                        string wordOnlySend = ExtractWord(textToSend);
                        SendText(wordOnlySend + end, input);
                        break;
                    }
                    else if (i == PageSize - 1)
                    {
                        ...
                        string textToSend = InputStatusFrm.cachearry[i].Split('|')[1];
                        string wordOnlySend = ExtractWord(textToSend);
                        SendText(wordOnlySend + end, input);
                        break;
                    }
                ...
                else if (tpos >= PageSize)
                {
                    if (!string.IsNullOrEmpty(InputStatusFrm.cachearry[PageSize - 1 - i]))
                    {
                        ...
                        string textToSend = InputStatusFrm.cachearry[PageSize - 1 - i].Split('|')[1];
                        string wordOnlySend = ExtractWord(textToSend);
                        SendText(wordOnlySend + end, input);
                        break;
                    }
                ...
                else
                {
                    if (!string.IsNullOrEmpty(InputStatusFrm.cachearry[tpos]))
                    {
                        ...
                        if (InputStatusFrm.Dream)
                            index = 0;
                        string textToSend = InputStatusFrm.cachearry[tpos].Split('|')[1];
                        string wordOnlySend = ExtractWord(textToSend);
                        SendText(wordOnlySend + end, input);
                        break;
                    }
                        ...
                        && tvar[0].Split('|')[2].Length > 0 && LastSPValue.Length > 1)
                    {
                        Task autoadd = new Task(() =>
                        {
                            try
                            {
                                ...
                                string localInputstr = this.pinputstr.Substring(0, 3);
                                string inputvalue = LastSPValue;
                                int pos1 = 0;
                                bool newadd = false;
                                string[] mdict = null;

                                PosIndex poi = Core.Win.WinInput.Input.DictIndex.GetPos(localInputstr, ref mdict, false);
                                if (poi == null)
                                {
                                    return;
                                }
                                ...
                                for (int i = first; i <= last; i++)
                                {
                                    ...
                                    if (mdict[i].Split(' ')[0] == localInputstr)
                                    {
                                ...
                                if (pos1 > 0)
                                {
                                    if (newadd)
                                    {
                                        ...
                                        if (inputvalue.Length == 1 && localInputstr.Length > 1)
                                            poi = Core.Win.WinInput.Input.DictIndex.GetPos(localInputstr.Substring(0, 2), ref mdict, false);//单字情况
                                        if (mdict == null) mdict = Core.Win.WinInput.Input.MasterDit;
                                        int posint = 0;
                                        for (int i = poi.Start; i <= poi.End; i++)
                                        {
                                            if (mdict[i].Split(' ')[1].Length == 1 && mdict[i].Split(' ')[0].Length > 1
                                                && mdict[i].Split(' ')[0].Substring(0, 2) == localInputstr.Substring(0, 2))
                                            {
                                                pos1 = i;
                                                if (posint == 0)
                                                    posint++;
                                        ...
                                        if (inputvalue.Length == 1 && localInputstr.Length > 1 && pos1 > poi.Start)
                                            pos1--;

                                        var list = Core.Win.WinInput.Input.MasterDit.ToList();
                                        list.Insert(pos1 + 1, localInputstr + " " + inputvalue);
        ...
        public InputStatusFrm(InputMode input)
        {
            ...
            // 订阅鼠标事件用于拖动
            this.MouseDown += InputStatusFrm_MouseDown;
            this.MouseMove += InputStatusFrm_MouseMove;
            this.MouseUp += InputStatusFrm_MouseUp;
        ...
        // 鼠标按下开始拖动
        private void InputStatusFrm_MouseDown(object sender, MouseEventArgs e)
        {
        ...
        // 不需要MouseMove来处理拖动了，Windows会自动处理
        private void InputStatusFrm_MouseMove(object sender, MouseEventArgs e)
        {
            // 空的，Windows会处理拖动
        }

        // 鼠标松开时保存位置（只有拖动时才保存）
        private void InputStatusFrm_MouseUp(object sender, MouseEventArgs e)
        {
            // 只有在鼠标左键松开时才保存位置
            if (e.Button == MouseButtons.Left)
            {
        ...
        {
            // 从 WinInput 中的 InputStatus 获取位置
            if (Win.WinInput.InputStatus.Left > 0 && Win.WinInput.InputStatus.Top > 0)
            {
                this.Left = Win.WinInput.InputStatus.Left;
                this.Top = Win.WinInput.InputStatus.Top;
            }
        }
        ...
        public void SaveCurrentPosition()
        {
            ...
            positionSaved = true;
            // 保存到配置文件
            Win.WinInput winInput = new Win.WinInput();
            winInput.SaveSetting();
        }
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            try
            {
                using (BufferedGraphics grafx = context.Allocate(e.Graphics, new Rectangle(0, 0, Width, Height)))
                {
                    ...
                    // 分别绘制四条边框线
                    Color originalColor = InputMode.Skinbordpen;
                    
                    // 创建暗10%亮度的颜色 - 简单方式
                    int r = Math.Max(0, originalColor.R - 50);
                    int g = Math.Max(0, originalColor.G - 50);
                    int b = Math.Max(0, originalColor.B - 50);
                    Color darkerColor = Color.FromArgb(originalColor.A, r, g, b);
                    
                    Pen originalPen = new Pen(originalColor);
                    Pen darkerPen = new Pen(darkerColor);
                    
                    // 上侧和右侧用原始颜色
                    grafx.Graphics.DrawLine(originalPen, 0, 0, Width - 1, 0); // 上侧
                    grafx.Graphics.DrawLine(originalPen, Width - 1, 0, Width - 1, Height - 1); // 右侧
                    
                    // 底部和左侧用暗10%的颜色
                    grafx.Graphics.DrawLine(darkerPen, 0, Height - 1, Width - 1, Height - 1); // 底部
                    grafx.Graphics.DrawLine(darkerPen, 0, 0, 0, Height - 1); // 左侧
                    ...
                    // 第一行编码在上边缘以下0.68个字高的位置显示
                    float codeLineY = InputMode.SkinFontH * 0.68f;
                    grafx.Graphics.DrawString(ins, new Font(InputMode.cffontname, fontsize > 18 ? 18 : fontsize), bstring, new Point(0 + 3, (int)codeLineY));

                    if (valuearry != null && valuearry.Length > 0 && !InputStatusFrm.Dream && valuearry.Length > PageSize) //分页数显示
                    {
                        float pageNumY = InputMode.SkinFontH * 0.68f;
                        grafx.Graphics.DrawString(string.Format("{0}/{1}", PageNum, valuearry.Length / PageSize + 1), new Font("", 11F), bstring, new Point(Width - 44, (int)pageNumY));
                    }

                    if (InputMode.ViewType == 0)
                    {
                        // 横排显示：使用原始的 inputy
                        inputy = InputMode.SkinFontJG;
                    }
                    else
                    {
                        // 竖排显示：顶部编码和第一个候选之间减去0.7个字距
                        float lineHeight_global = InputMode.SkinFontH * 1.35f;
                        float subtractSpacing = InputMode.SkinFontH * 0.7f;
                        inputy = InputMode.SkinFontJG + (int)lineHeight_global - (int)subtractSpacing;
                    }
                    ...
                    if (InputMode.ViewType == 0)
                    {
                        ...
                        for (int i = 0; i < cachearry.Length; i++)
                        {
                            try
                            {
                                ...
                                // 横排显示：编码提示水平位置下降4个像素
                                grafx.Graphics.DrawString(cachearry[i].Split('|')[2], new Font("", fontsize - 1), bcstring, new Point(wx + 8, inputy));
                    ...
                    {
                        //竖排显示
                        float lineHeight_v = InputMode.SkinFontH * 1.35f;
                        for (int i = 0; i < cachearry.Length; i++)
                        {
                            try
                            {
                                if (InputMode.lbinputv == null || InputMode.lbinputv[i] == null) break;
                                if (string.IsNullOrEmpty(cachearry[i])) break;
                                string v = GetCutStr(cachearry[i].Split('|')[1]);
                                string pos = i == 9 ? "0." : (i + 1).ToString() + ".";
                                ...
                                // 使用截断后的词计算宽度
                                string truncatedWord = GetCutStr(cachearry[i].Split('|')[1]);
                                int vw = InputMode.GetWidth(truncatedWord);
                                
                                InputMode.lbinputv[i].Text = pos + v;
                                
                                int yPos = inputy + (int)(i * lineHeight_v);
                                
                                if (i == 0)
                                    grafx.Graphics.DrawString(pos + v, tfont, fbcstring, new Point(3, yPos));
                                else
                                    grafx.Graphics.DrawString(pos + v, tfont, bstring, new Point(3, yPos));

                                // 提示文本显示在字词右侧，避免重叠，调整高度使其一致
                                string hintText = cachearry[i].Split('|')[2];
                                if (!string.IsNullOrEmpty(hintText))
                                {
                                    grafx.Graphics.DrawString(hintText, new Font("宋体", InputMode.SkinFontSize - 1), bcstring, new Point(3 + vw + 10, yPos + 2));
                                }
                            }
                            catch { }
                        }
