...
namespace Core.Base
{
    ...
    public class InputMode
    {
        ...
        private Dictionary<string, string> hzToCodeDict = new Dictionary<string, string>();
        ...
        public bool IsShiftEnglishMode = false; // Shift英文输入模式标志
        public bool IsFirstCharInShiftMode = true; // Shift模式下是否是第一个字符
        ...
        public static int PageSize = 5; //候选框数量
        ...
        public static bool tautopos = true;//码表调频
        ...
        public static bool semicolonSelect = false;//开启 ;'/选重
        ...
        public static int ViewType = 0;//0横排,1竖排
        ...
        public virtual string[] GetInputValue(string inputstr, bool dream = false,int ncount=0,bool isright=false)
        {
            ...
            {
                if (inputstr.Length == 1 && IsChinese == 1 && inputstr!="'" && !IsShiftEnglishMode)
                {
            ...
            try
            {
                if (IsChinese == 1)
                {
                    ...
                    if (!inputstr.StartsWith("'"))
                    {
                        ...
                        #region 自动调频
                        string[] mdict = MasterDit;
                        if (indexComplete)
                        {
                            // 直接使用MasterDit数组，不通过GetPos获取
                            // 这样可以确保我们修改的是原始数组
                        }
                        else return false;
                        ...
                        int totalWords = 0;
                        List<int> wordCounts = new List<int>(); // 记录每个记录中的词数
                        ...
                        for (int i = 0; i < mdict.Length; i++)
                        {
                            ...
                            {
                                positions.Add(i);
                                int wordCount = mdict[i].Split(' ').Length - 1; // 减去编码部分
                        ...
                        {
                            int recordIndex = positions[i];
                            string[] parts = mdict[recordIndex].Split(' ');
                            int wordCount = parts.Length - 1;
                        ...
                        if (targetRecordIndex == -1 || targetWordIndex == -1) return false;
                        
                        // 处理同一记录中字词的顺序调整
                        string[] recordParts = mdict[targetRecordIndex].Split(' ');
                        if (recordParts.Length > 2 && targetWordIndex > 1)
                        {
                            // 保存目标词
                            string targetWord = recordParts[targetWordIndex];
                            // 将目标词移动到记录的开头
                            for (int i = targetWordIndex; i > 1; i--)
                            {
                                recordParts[i] = recordParts[i - 1];
                            }
                            recordParts[1] = targetWord;
                            // 重新组合记录
                            mdict[targetRecordIndex] = string.Join(" ", recordParts);
                        }
                        else if (positions.Count > 1)
                        {
                            ...
                            {
                                string temp = mdict[targetPos];
                                for (int i = targetPos; i > firstPos; i--)
                                {
                                    mdict[i] = mdict[i - 1];
                                }
                                mdict[firstPos] = temp;
                        ...
                        try
                        {
                            if (!string.IsNullOrEmpty(MasterDitPath))
                            {
                                // 保存码表到文件
                                System.IO.File.WriteAllLines(MasterDitPath, MasterDit, System.Text.Encoding.UTF8);
                                // 重新创建索引，确保缓存中的顺序也更新
                                if (indexComplete)
                                {
                                    DictIndex.IndexList.Clear();
                                    CreateIndex(MasterDit, ref DictIndex.IndexList, 1, 0, MasterDit.Length);
                                }
                            }
                        }
                        catch { }
        ...
        /// <param name="hz"></param>
        /// <returns></returns>
        private string GetOriginalCode(string hz)
        {
            string code;
            if (hzToCodeDict.TryGetValue(hz, out code))
            {
                return code;
            }
            return "";
        ...
        /// <param name="hz"></param>
        /// <returns></returns>
        public string GetCodeForChar(string hz)
        {
            return GetOriginalCode(hz);
        }

        /// <summary>
        /// 构建汉字到编码的映射字典
        /// </summary>
        public void BuildHzToCodeDict()
        {
            hzToCodeDict.Clear();
            if (MasterDit != null)
            {
                foreach (var item in MasterDit)
                {
                    ...
                    for (int i = 1; i < parts.Length; i++)
                    {
                        string hz = parts[i];
                        // 只保存单字的编码
                        if (hz.Length != 1) continue;
                        
                        // 如果已经有编码，只保存更长的编码（优先使用4码编码）
                        if (!hzToCodeDict.ContainsKey(hz))
                        {
                            hzToCodeDict[hz] = code;
                        }
                        else
                        {
                            // 如果新编码更长，更新为更长的编码
                            if (code.Length > hzToCodeDict[hz].Length)
                            {
                                hzToCodeDict[hz] = code;
                            }
                        }
        ...
        private void GetCloudDict(string inputstr, ref string valuestr)
        {
            if (inputstr.StartsWith("'"))
            {
                ...
                foreach (var s in ClouddDit.FindAll(f => f.StartsWith(inputstr)))
                {
                    ...
                    {
                        string hz = s.Split(' ')[i];
                        if (valuestr.IndexOf("|" + hz + "|") < 0)
                        {
                            string originalCode = GetOriginalCode(hz);
                            if (!string.IsNullOrEmpty(originalCode))
                            {
                                valuestr += "|" + hz + "[" + originalCode + "]|" + s.Split(' ')[0].Replace(inputstr, "") + "\n";
                            }
                            else
                            {
                                valuestr += "|" + hz + "|" + s.Split(' ')[0].Replace(inputstr, "") + "\n";
                            }
        ...
        public string CheckKeysString(Keys keys)
        {
            ...
            // 当处于Shift英文模式时，默认使用半角英文标点
            bool originalIsCnBd = this.IsCnBd;
            if (this.IsShiftEnglishMode)
            {
                this.IsCnBd = false;
            }
            string str = "";
        ...
        private void ShowWork()
        {
            while (!Break)
            {
                ...
                try
                {
                    ...
                    {
                        //横排显示：候选框最底部编码与底部距离设为与编码距顶部相同(0.68倍字高)
                        float lineHeight = InputMode.SkinFontH * 1.35f;
                        float subtractSpacing = InputMode.SkinFontH * 0.7f;
                        Win.WinInput.InputStatus.Width = Width = MaxWidth();
                        Win.WinInput.InputStatus.Height = Height = (int)(InputMode.SkinHeith + lineHeight - subtractSpacing + InputMode.SkinFontH * 0.68f);
                        //Win.WinInput.ImageInput.Width = ImageInput.MaxWidth();
                    }
                    else
                    {
                        //竖排显示：候选框最底部编码与底部距离设为与编码距顶部相同(0.68倍字高)
                        float lineHeight = InputMode.SkinFontH * 1.35f;
                        float subtractSpacing = InputMode.SkinFontH * 0.7f;
                        Win.WinInput.InputStatus.Height = Height = (int)((MaxHeight() + SkinFontJG + lineHeight - subtractSpacing + InputMode.SkinFontH * 0.68f) * 1.25);
                        Win.WinInput.InputStatus.Width = Width = 135;
                    }
                    if (!this.Metor)
                    {
