...
namespace Core.Win
{
    ...
    public class WinInput :System.Windows.Forms.Form,IOut
    {
        ...
        //int Aa = 0;
        //int AA = 0;
        private ConfigFrm configForm = null;
        ...
        public bool LoadSettting()
        {
            ...
            InputMode.SkinBack = string.IsNullOrEmpty(SetInfo.GetValue("SkinBack", setting)) ? Color.FromArgb(22, 79, 141) : Color.FromArgb(int.Parse(SetInfo.GetValue("SkinBack", setting)));
        ...
        private int KeyboardHookProc(int nCode, Int32 wParam, IntPtr lParam)
        {
            ...
            try
            {
                ...
                if ((wParam == WM_KEYDOWN || wParam == WM_SYSKEYDOWN))
                {
                    ...
                    // Alt+k 快捷键打开/关闭设置界面
                    if (Input.IsPressAlt && keyData == Keys.K && Input.isActiveInput)
                    {
                        if (configForm == null || configForm.IsDisposed)
                        {
                            configForm = new ConfigFrm(this);
                            configForm.Show();
                        }
                        else
                        {
        ...
        private int UserOnKeyDown(Keys key)
        {
            ...
            // 首先检查是否是功能键，如果是，直接返回0，让按键传递给下一个应用程序
            if (Comm.Function.FunionKey(key))
            {
                return 0;
            }
        ...
        private void UserOnKeyUp()
        {
            ...
            if (queuecount == 1)
            {
                ...
                if (_lkey.KeyData == Keys.OemSemicolon || _lkey.KeyData == Keys.Oem7)
                {
        ...
        private void SetCurPos()
        {
            ...
            {
                // 未启用光标/鼠标跟随，使用保存的位置
                InputStatus.RestoreSavedPosition();
            }
        }
        #endregion
        /// <summary>
        ...
        private void Initializenotifyicon()
        {
            ...
            TrayIcon.Text = "速录宝3.2.5";//鼠标移至托盘的提示文本
            TrayIcon.Visible = true;

            //定义一个MenuItem数组，并把此数组同时赋值给ContextMenu对象 
            mnuItms = new MenuItem[14];
            mnuItms[mnuItms.Length - 14] = new MenuItem();
            mnuItms[mnuItms.Length - 14].Text = "关于速录宝3.2.5";
